...
import { WalletIcon, CrossIcon } from "./icons";
  ...
  if (user.connected) {
    return (
      <div className="relative">
        ...
        >
          <span className="hidden font-mono text-xs text-accent-soft tnum sm:inline">
            {usdc(user.usdc)}
          </span>
          <span className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-accent to-accent-dim text-[10px] font-bold text-white">
          ...
          {menu && (
            <>
              ...
              <motion.div
                ...
                className="absolute right-0 z-50 mt-2 w-64 rounded-xl border border-edge bg-elevated p-3 shadow-2xl"
              >
                ...
                <div className="mt-3 rounded-lg border border-edge bg-white/[0.02] p-3">
                  <p className="text-[11px] uppercase tracking-wide text-muted">Wallet</p>
                  <p className="font-mono text-lg font-semibold tnum">
                    {usdc(user.usdc)} <span className="text-sm text-muted">USDC</span>
                  </p>
                </div>
                <button
                  type="button"
                ...
                <p className="mt-2 text-[10px] leading-relaxed text-muted/70">
                  Disconnecting clears the demo positions along with the account.
                </p>
              </motion.div>
            </>
          )}
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-white transition-shadow hover:shadow-[0_0_24px_rgba(168,85,247,0.5)]"
      >
                ...
                {[
                  ...
                  >
                    ...
                    <span className="flex-1">
                      ...
                      {w.tag && <span className="block text-xs text-accent-soft">{w.tag}</span>}
                    </span>
                  </button>
                ))}
              </div>
              <p className="mt-4 text-center text-[11px] leading-relaxed text-muted">
                Demo mode — connecting funds a mock account with 5,000 USDC. No
